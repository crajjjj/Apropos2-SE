...
Function RegisterForEvents()
    ...
    RegisterForModEvent("APRWT_ApplyWearAndTear", "ApplyWearAndTear") ;exists but not used?
...
Function DoApplyWearAndTear(Actor anActor, String area, string damageClass, Bool isRapeOrAggressive, Bool isCreature = false, Bool intenseMod = true, Bool hugeMod = false)
;Function DoApplyWearAndTear(Actor anActor, String area, string damageClass, Bool isRapeOrAggressive, Bool isCreature)
    ...
    String actorName = anActor.GetDisplayName()
    If Config.TraceMessagesEnabled
        Debug("DoApplyWearAndTear(" + actorName + ","+ area + "," + damageClass + "," + isRapeOrAggressive + ")")
    EndIf
    ...
    Apropos2ActorAlias aproposActor = GetTrackedActorOrStartTracking(anActor)
    If aproposActor
        Int amount = GetWearTearDamage(damageClass, isRapeOrAggressive, intenseMod, hugeMod)
        If Config.TraceMessagesEnabled
            Debug("DoApplyWearAndTear - amount:(" + amount + ")")
        EndIf
        If area == "Vaginal"
...
Event ApplyWearAndTear(Form frm, String area, string damageClass, Bool isRapeOrAggressive, Bool isCreature)
    ...
    DoApplyWearAndTear(anActor, area, damageClass, isRapeOrAggressive, isCreature,  true,  false)
EndEvent
...
Int Function GetWearTearDamage(String damageClass, Bool isRapeOrAggressive, Bool intenseMod, Bool hugeMod)
    If Config.TraceMessagesEnabled
    ...
    If isRapeOrAggressive
        Float afactor = JDB.solveFlt(QueryKey("wearteardamage", "AggressiveOrRape"), 1.1)
        amt = (amt * afactor)
    EndIf
    
    If (intenseMod && hugeMod)
         ; fallback multiplier
         amt = (amt * 4)
    elseif hugeMod
        Float hfactor = JDB.solveFlt(QueryKey("wearteardamage", "HugeModifier"), 1.2)
        amt = (amt * hfactor)
    elseif intenseMod
        Float ifactor = JDB.solveFlt(QueryKey("wearteardamage", "IntenseModifier"), 1.0)
        amt = (amt * ifactor)
    else
        Float sfactor = JDB.solveFlt(QueryKey("wearteardamage", "SlowModifier"), 0.8)
        amt = (amt * sfactor)
    EndIf
