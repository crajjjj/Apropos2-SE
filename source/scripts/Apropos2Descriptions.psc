...
Function DisplayAnimationStartChangeOrStageStartMessage(SslThreadController thread, Int stage, Bool isAnimationChange)
    ...
    EndIf
    int pos = GetActorPositionFromList(actorList, primaryActor)
    ...
    ElseIf !isLesbianSex && isPrimaryFemale && !isCreatureSex; && !isThreeWay
        ...
        ElseIf isVaginal && (isVaginalPenetrated || isDP)
        ...
        If !animation.HasTag("Lesbian"); we are using a regular MF animation probably but with two Fs
            ...
            If isVaginal || isAnal || isBoobJob || (isOral && !isCunnilingus); look for tags that seem to indicate a strapon
                ...
                If actorWithStrapOnIndex != -1 ; if the animation is tagged as vaginal or anal, and we are all females, we should have a strapon somewhere ...
                    ...
                    If isVaginal && (isVaginalPenetrated || isDP)
...
Function ProcessStageStartWT(SslThreadController thread, int Stage)
    ...
    Bool isThreeWay = animation.HasTag("Orgy"); captures MMF, FMM, FFM, MFF
    Bool isCreatureSex = thread.HasCreature || animation.HasTag("Creature")
    Bool isLesbianSexB = IsLesbianSex(animation, actorList)
    ...
    Bool isCunnilingusB = animation.HasTag("Cunnilingus") || (isOral && !isBlowjob)
    Bool isCreatureGangBangB = IsCreatureGangBang(animation)
    Bool isEstrusSex = animation.HasTag("Estrus")
    String maleGangBangSpecifier = GetGangBangSpecifier(animation)
    ...
    Bool isPrimaryMale = !isPrimaryFemale
    Bool isPrimaryVictim = thread.IsVictim(primaryActor)
    Bool isCummedInside = IsCummedInside(endingLabel)
    ...
    ElseIf !isLesbianSexB && isPrimaryFemale && !isCreatureSex; && !isThreeWay
        ...
        If IsGettingVaginallyPenetrated(penetrationLabel)
            ActorsLib.DoApplyWearAndTear(primaryActor, VAGINAL, damageClass, hasVictimOrAggressive, isCreatureSex, isIntensePen, false)
            needUpdate=true
        endif
        if IsGettingAnallyPenetrated(penetrationLabel)
            ActorsLib.DoApplyWearAndTear(primaryActor, ANAL, damageClass, hasVictimOrAggressive, isCreatureSex, isIntensePen, false)
            needUpdate=true
        EndIf
        if IsGettingDoublePenetrated(penetrationLabel)
            ActorsLib.DoApplyWearAndTear(primaryActor, DP, damageClass, hasVictimOrAggressive, isCreatureSex, isIntensePen, false)
            needUpdate=true
        EndIf
        if isCummedInside
             if isAnal
                ActorsLib.DoApplyWearAndTear(primaryActor, ANAL, damageClass, hasVictimOrAggressive, isCreatureSex, false, false)
                needUpdate=true
            ElseIf isVaginal  
                ActorsLib.DoApplyWearAndTear(primaryActor, VAGINAL, damageClass, hasVictimOrAggressive, isCreatureSex, false, false)
                needUpdate=true
            ElseIf isOral
                ActorsLib.DoApplyWearAndTear(primaryActor, ORAL, damageClass, hasVictimOrAggressive, isCreatureSex, false, false)
                needUpdate=true
            endif
        EndIf
        if isFisting
            if isAnal
                ActorsLib.DoApplyWearAndTear(primaryActor, ANAL, damageClass, hasVictimOrAggressive, isCreatureSex, false, true)
                needUpdate=true
            ElseIf isVaginal
                ActorsLib.DoApplyWearAndTear(primaryActor, VAGINAL, damageClass, hasVictimOrAggressive, isCreatureSex, false, true)
                needUpdate=true
            endif
        EndIf
        if IsSuckingoffOther(oralLabel)
            ActorsLib.DoApplyWearAndTear(primaryActor, ORAL, damageClass, hasVictimOrAggressive, false, isIntenseOral,false)
            needUpdate=true
        EndIf
        if needUpdate
            ActorsLib.DoUpdateEffectsAndTats(primaryActor, increasingAbuse=True)  
            Deflower(primaryActor, IsGettingVaginallyPenetrated(penetrationLabel), IsGettingAnallyPenetrated(penetrationLabel), IsSuckingoffOther(oralLabel))
        endif
    ; Creature and Female PC
    ElseIf isCreatureSex && isPrimaryFemale
        Debug(">> Creature and Female PC or NPC")
        ...
        If IsGettingVaginallyPenetrated(penetrationLabel)
            ActorsLib.DoApplyWearAndTear(primaryActor, VAGINAL, creatureType, hasVictimOrAggressive, isCreatureSex, isIntensePen,false)
            needUpdate=true
        endif
        if IsGettingAnallyPenetrated(penetrationLabel)
            ActorsLib.DoApplyWearAndTear(primaryActor, ANAL, creatureType, hasVictimOrAggressive, isCreatureSex,isIntensePen, false)
            needUpdate=true
        EndIf
        if IsGettingDoublePenetrated(penetrationLabel)
            ActorsLib.DoApplyWearAndTear(primaryActor, DP, creatureType, hasVictimOrAggressive, isCreatureSex, isIntensePen, false)
            needUpdate=true
        EndIf
        if isStimulated
            if isAnal
                ActorsLib.DoApplyWearAndTear(primaryActor, ANAL, creatureType, hasVictimOrAggressive, isCreatureSex, false, false)
                needUpdate=true
            ElseIf isVaginal  
                ActorsLib.DoApplyWearAndTear(primaryActor, VAGINAL, creatureType, hasVictimOrAggressive, isCreatureSex, false, false)
                needUpdate=true
            endif
        endif
        if isCummedInside
             if isAnal
                ActorsLib.DoApplyWearAndTear(primaryActor, ANAL, creatureType, hasVictimOrAggressive, isCreatureSex, true, false)
                needUpdate=true
            ElseIf isVaginal  
                ActorsLib.DoApplyWearAndTear(primaryActor, VAGINAL, creatureType, hasVictimOrAggressive, isCreatureSex, true, false)
                needUpdate=true
            ElseIf isOral
                ActorsLib.DoApplyWearAndTear(primaryActor, ORAL, creatureType, hasVictimOrAggressive, isCreatureSex, true, false)
                needUpdate=true
            endif
        EndIf
        if isFisting
            if isAnal
                ActorsLib.DoApplyWearAndTear(primaryActor, ANAL, creatureType, hasVictimOrAggressive, isCreatureSex, false, true)
                needUpdate=true
            ElseIf isVaginal  
                ActorsLib.DoApplyWearAndTear(primaryActor, VAGINAL, creatureType, hasVictimOrAggressive, isCreatureSex, false, true)
                needUpdate=true
            endif
        EndIf
        if IsSuckingoffOther(oralLabel)
            ActorsLib.DoApplyWearAndTear(primaryActor, ORAL, creatureType, hasVictimOrAggressive, isCreatureSex, isIntenseOral, false)
            needUpdate=true
        EndIf
    ...
    ; Primary Straight Male PC and Female
    ElseIf !isLesbianSexB && !isCreatureSex && isPrimaryMale && !isGaySex && !isThreeWay
        Debug(">> Primary Straight Male PC and Female")
        Actor femaleActor = actorList[0] ; in a two-way MF, Female is always position 0 according to Ashal
        Bool isFemaleVictim = thread.IsVictim(femaleActor)
        Bool isIntenseB = isIntense(penetrationLabel)
        ;Bool isPrimaryDremora = IsDremora(primaryActor)
        Bool hasVictimOrAggressive = isFemaleVictim || isAggressive
        ...
        If IsGettingVaginallyPenetrated(penetrationLabel)
            ActorsLib.DoApplyWearAndTear(femaleActor, VAGINAL, damageClass, hasVictimOrAggressive, false, isIntenseB , false)
        endif
        if IsGettingAnallyPenetrated(penetrationLabel)
            ActorsLib.DoApplyWearAndTear(femaleActor, ANAL, damageClass, hasVictimOrAggressive, false, isIntenseB, false)
        EndIf
        if IsGettingDoublePenetrated(penetrationLabel)
            ActorsLib.DoApplyWearAndTear(femaleActor, DP, damageClass, hasVictimOrAggressive, false, isIntenseB, false)
        EndIf
        if isFisting
            if isAnal
                ActorsLib.DoApplyWearAndTear(femaleActor, ANAL, damageClass, hasVictimOrAggressive, false, false, true)
            ElseIf isVaginal  
                ActorsLib.DoApplyWearAndTear(femaleActor, VAGINAL, damageClass, hasVictimOrAggressive, false, false, true)
            endif
        EndIf
        if IsSuckingoffOther(oralLabel)
            ActorsLib.DoApplyWearAndTear(femaleActor, ORAL, damageClass, hasVictimOrAggressive, false, isIntense(OralLabel), false)
        EndIf
        if needUpdate
             ActorsLib.DoUpdateEffectsAndTats(femaleActor, increasingAbuse=True)
             Deflower(femaleActor, IsGettingVaginallyPenetrated(penetrationLabel), IsGettingAnallyPenetrated(penetrationLabel), IsSuckingoffOther(oralLabel))
        endif
    ElseIf isLesbianSexB ; defined as two females
        Debug(">> Lesbian sex")
        If !animation.HasTag("Lesbian")  ; we are using a regular MF animation probably but with two Fs
            Debug("... not tagged as Lesbian")
            If isVaginal || isAnal || (isOral && !isCunnilingusB) ; look for tags that seem to indicate a strapon
                Int actorWithStrapOnIndex = GetIndexOfMalePosition(thread, actorList)
                ...
                If actorWithStrapOnIndex != -1 ; if the animation is tagged as vaginal or anal, and we are all females, we should have a strapon somewhere ...
                    ...
                    Bool isReceivingVictim = thread.IsVictim(receivingActor)
                      ;Bool isPrimaryDremora = IsDremora(primaryActor)
                    Bool hasVictimOrAggressive = isReceivingVictim || isAggressive
                    String damageClass = "Human"
                    If IsDremora(primaryActor)
                        damageClass = "Dremora"
                    EndIf
                    ...
                    If IsGettingVaginallyPenetrated(penetrationLabel)
                           ActorsLib.DoApplyWearAndTear(receivingActor, VAGINAL, damageClass, hasVictimOrAggressive, false, isIntense(penetrationLabel),false)
                           needUpdate=true
                    endif
                    if IsGettingAnallyPenetrated(penetrationLabel)
                         ActorsLib.DoApplyWearAndTear(receivingActor, ANAL, damageClass, hasVictimOrAggressive, false, isIntense(penetrationLabel),false)
                         needUpdate=true
                    EndIf
                    if IsGettingDoublePenetrated(penetrationLabel)
                        ActorsLib.DoApplyWearAndTear(receivingActor, DP, damageClass, hasVictimOrAggressive, false, isIntense(penetrationLabel),false)
                        needUpdate=true
                    EndIf
                     if IsGettingInsertedBig(stimulationLabel)
                         if isAnal
                             ActorsLib.DoApplyWearAndTear(receivingActor, ANAL, damageClass, hasVictimOrAggressive, false, false, true)
                             needUpdate=true
                        ElseIf isVaginal  
                            ActorsLib.DoApplyWearAndTear(receivingActor, VAGINAL, damageClass, hasVictimOrAggressive, false, false, true)
                            needUpdate=true
                        endif
                     EndIf
                     if IsSuckingoffOther(oralLabel)
                        ActorsLib.DoApplyWearAndTear(receivingActor, ORAL, damageClass, hasVictimOrAggressive, false, isIntense(OralLabel), false)
                        needUpdate=true
                     EndIf

                     if needUpdate
                        ActorsLib.DoUpdateEffectsAndTats(receivingActor, increasingAbuse=True)
                        ...
                        Deflower(receivingActor, IsGettingVaginallyPenetrated(penetrationLabel), IsGettingAnallyPenetrated(penetrationLabel), IsSuckingoffOther(oralLabel))
                     endif
                EndIf
            ElseIf isForeplay
                ; who is primary and who is active? Gonna just use the default answers: that 0-PlayerRef, 1-other chick
                ;DisplayFemaleActorFemaleForeplayDescriptions(thread, isPrimaryVictim, activeActor, primaryActor, effectiveVoice, True)
            ElseIf isCunnilingusB
                ; who is primary and who is active? Gonna just use the default answers: that 0-PlayerRef, 1-other chick
...
Function ProcessAnimEndWT(SslThreadController thread, int Stage) 
    ...
    If (thread.HasPlayer)
        primaryActor = PlayerRef
        pos = GetActorPositionFromList(actorList, PlayerRef)
    Else
        primaryActor = actorList[0]
        pos=0
    EndIf
    ...
    ElseIf !isLesbianSex && isPrimaryFemale && !isCreatureSex; && !isThreeWay
        ...
        If maleGangBangSpecifier != "" 
            If isVaginal
                ...
                ActorsLib.DoApplyWearAndTear(primaryActor, VAGINAL, damageClass, hasVictimOrAggressive, isCreature=False, intenseMod = true, hugeMod = true)
            EndIf
            If isAnal
                ;Framework.ApplyAnalWearAndTear(primaryActor, "Human", isPrimaryVictim || isAggressive, isDremora, False)
                ActorsLib.DoApplyWearAndTear(primaryActor, ANAL, damageClass, hasVictimOrAggressive, isCreature=False, intenseMod = true, hugeMod = true)
            EndIf
            If isOral
                ;Framework.ApplyOralWearAndTear(primaryActor, "Human", isPrimaryVictim || isAggressive, isDremora, False)
                ActorsLib.DoApplyWearAndTear(primaryActor, ORAL, damageClass, hasVictimOrAggressive, isCreature=False, intenseMod = true, hugeMod = true)                
            EndIf
    ...
    ElseIf isCreatureSex && isPrimaryFemale
        ...
        If isCreatureGangBang
            ...
            If isVaginal
                ActorsLib.DoApplyWearAndTear(primaryActor, VAGINAL, creatureType, isPrimaryVictim || isAggressive, isCreature=True, intenseMod = true, hugeMod = true)
                ;Framework.ApplyVaginalWearAndTear(primaryActor, creatureType, isPrimaryVictim || isAggressive, isDaedric, True)
            EndIf
            If isAnal
                ActorsLib.DoApplyWearAndTear(primaryActor, ANAL, creatureType, isPrimaryVictim || isAggressive, isCreature=True, intenseMod = true, hugeMod = true)
                ;Framework.ApplyAnalWearAndTear(primaryActor, creatureType, isPrimaryVictim || isAggressive, isDaedric, True)
            EndIf
            If isOral
                ActorsLib.DoApplyWearAndTear(primaryActor, ORAL, creatureType, isPrimaryVictim || isAggressive, isCreature=True, intenseMod = true, hugeMod = true)
    ...
    ElseIf !isLesbianSex && !isCreatureSex && isPrimaryMale && !isGaySex && !isThreeWay
        ...
        If isVaginal
            ...
            ActorsLib.DoApplyWearAndTear(femaleActor, VAGINAL, damageClass, hasVictimOrIsAggressive, isCreature=False,intenseMod = true, hugeMod = true)
        ElseIf isAnal
            ...
            ActorsLib.DoApplyWearAndTear(femaleActor, ANAL, damageClass, hasVictimOrIsAggressive, isCreature=False,intenseMod = true, hugeMod = true)
        ElseIf isOral
            ...
            ActorsLib.DoApplyWearAndTear(femaleActor, ORAL, damageClass, hasVictimOrIsAggressive, isCreature=False,intenseMod = true, hugeMod = true)
        ElseIf isBoobJob
        ...
        If !animation.HasTag("Lesbian")  ; we are using a regular MF animation probably but with two Fs
            ...
            If isVaginal || isAnal || isBoobJob || (isOral && !isCunnilingus) ; look for tags that seem to indicate a strapon
                ...
                If actorWithStrapOnIndex != -1 ; if the animation is tagged as vaginal or anal, and we are all females, we should have a strapon somewhere ...
                    ...
                    Bool isReceivingVictim = thread.IsVictim(receivingActor)
                      ;Bool isPrimaryDremora = IsDremora(primaryActor)
                    Bool hasVictimOrIsAggressive = isReceivingVictim || isAggressive
                    String damageClass = "Human"
                    If IsDremora(primaryActor)
                        damageClass = "Dremora"
                    EndIf
                    ...
                    If isVaginal
                        ActorsLib.DoApplyWearAndTear(receivingActor, VAGINAL, damageClass, hasVictimOrIsAggressive, isCreature=False,intenseMod = true, hugeMod = true)
                    ...
                    ElseIf isAnal
                        ActorsLib.DoApplyWearAndTear(receivingActor, ANAL, damageClass, hasVictimOrIsAggressive, isCreature=False,intenseMod = true, hugeMod = true)
                    ...
                    ElseIf isOral
                         ActorsLib.DoApplyWearAndTear(receivingActor, ORAL, damageClass, hasVictimOrIsAggressive, isCreature=False,intenseMod = true, hugeMod = true)
                    ElseIf isBoobJob
