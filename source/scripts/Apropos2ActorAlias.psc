...
Function SetWTMorphs()
	if !Config.WearTearMorph
		return
	endif
	Actor anActor = GetActor()	
	NiOverride.SetBodyMorph(anActor,WTMorphName("v1"), "WTMorph",0.111*_vaginalState)
	NiOverride.SetBodyMorph(anActor,WTMorphName("v2"), "WTMorph",0.111*_vaginalState)
	NiOverride.SetBodyMorph(anActor,WTMorphName("a1"), "WTMorph",0.111*_analState)
	If(anActor.Is3dLoaded())
		NiOverride.UpdateModelWeight(anActor)
	EndIf
EndFunction

Function ClearWTMorphs()
	Actor anActor = GetActor()
	NiOverride.ClearBodyMorphKeys(anActor,"WTMorph")
	If(anActor.Is3dLoaded())
		NiOverride.UpdateModelWeight(anActor)
	EndIf
EndFunction

String Function WTMorphName(String name)

    String fullPath = GenerateFullPath("WearAndTear_Morphs")
    If CheckFileExists(fullPath)
        int jobj = JValue.readFromFile(fullPath)

        If jobj != 0 
            JValue.retain(jobj)
            String MorphName = JMap.getStr(jobj, name)
            JValue.release(jobj)
            Return MorphName
        EndIf
    EndIf
